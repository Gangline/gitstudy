...
//		String.format("서식문자를 포함한 문자열",[서식문자를 대체할 데이터]...);
		...
		//서식 문자 : 변환할 데이터의 형식(데이터타입)을 지정하는 문자
		// 데이터를 서식문자의 자리로 치환한다
		...
		String text2 = String.format("총점 %d 점 : ", 123); //데이터 앞에 0이 붙으면 8진수, 0x가 붙으면 16진수로 인식
		System.out.println( text2 );
		
		System.out.println( String.format("%s %d", "점수는" , 38) );
		
		//전체 문자열을 9자리로 구성해서 서식화한다
		//소수부분 4자리를 고정으로 사용한다
		//실수일 경우 .(소수점) 도 한 자리 차지한다
		//소수부, 소수점 빼고 나머지는 정수가 차지한다
		System.out.println( String.format("%9.4f", 123.45));	// = 123.4566
...
//		System.out.printf(	,	);
		...
		//	printf 메소드를 이용할 때 String.format 과 같은 방법으로 적용한다
